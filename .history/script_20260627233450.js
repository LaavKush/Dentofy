...
function toggleFaq(idx) {
  const targetItem = document.getElementById(`faq-${idx}`);
  const isAlreadyOpen = targetItem.classList.contains('open');

  // Downward Arrow SVG code block string
  const chevronDownSvg = `<svg class="chevron-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><polyline points="6 9 12 15 18 9"></polyline></svg>`;
  
  // Upward Arrow SVG code block string
  const chevronUpSvg = `<svg class="chevron-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" width="14" height="14"><polyline points="18 15 12 9 6 15"></polyline></svg>`;

  // Collapse all accordion modules completely
  document.querySelectorAll('.faq-item').forEach(el => {
    el.classList.remove('open');
    const chevronBtn = el.querySelector('.faq-chevron');
    if (chevronBtn) chevronBtn.innerHTML = chevronDownSvg;
  });

  // Activate and shift the current card open if it was closed
  if (!isAlreadyOpen) {
    targetItem.classList.add('open');
    const activeChevronBtn = targetItem.querySelector('.faq-chevron');
    if (activeChevronBtn) activeChevronBtn.innerHTML = chevronUpSvg;
  }
}
