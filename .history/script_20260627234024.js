...
function toggleFaq(idx){
  ...
  // Close all open items and reset their chevrons to down
  document.querySelectorAll('.faq-item').forEach(el => {
    el.classList.remove('open');
    const icon = el.querySelector('.faq-chevron i');
    if (icon) {
      icon.className = 'bi bi-chevron-down';
    }
  });
 
  // If the clicked item was closed, open it and change chevron to up
  if(!isOpen){
